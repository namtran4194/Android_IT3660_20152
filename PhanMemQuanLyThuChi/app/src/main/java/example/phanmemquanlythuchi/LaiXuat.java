...
class dbLaiXuat extends SQLiteOpenHelper {
	...
	private static final String STRING_CREATE = "CREATE TABLE " + TABLE_NAME
			+ " (_id INTEGER PRIMARY KEY AUTOINCREMENT," + COL_NAME + " TEXT," + COL_TIEN + " TEXT," + COL_LAIXUAT
			+ " TEXT," + COL_DATE + " TEXT);";
...
public class LaiXuat extends Activity {
	TextView ngaythang, tennganhang, tiennganhang, laixuat;
	ImageButton save;
		...
		save.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (tennganhang.getText().toString().isEmpty()) {
					...
					toast.show();
				} else if (tiennganhang.getText().toString().isEmpty()
						|| checkZero(tiennganhang.getText().toString())) {
					...
					mCursorlaixuat = mDblaixuat.query(dbLaiXuat.TABLE_NAME, columns, null, null, null, null, null,
							null);
					String[] headers = new String[] { dbLaiXuat.COL_NAME, dbLaiXuat.COL_DATE };
					mAdapter = new SimpleCursorAdapter(LaiXuat.this, R.layout.t_custom_listlaixuat, mCursorlaixuat,
							headers, new int[] { R.id.tv_custom_tenlaixuat, R.id.tv_custom_ngaylaixuat });
			...
			public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						img_savesua.setOnClickListener(new OnClickListener() {
							...
							public void onClick(View v) {
								...
								mDblaixuat.update(dbLaiXuat.TABLE_NAME, cv,
										"_id " + "=" + arrngay.get(position).getId(), null);
								String[] columns = new String[] { "_id", dbLaiXuat.COL_NAME, dbLaiXuat.COL_DATE };
								mCursorlaixuat = mDblaixuat.query(dbLaiXuat.TABLE_NAME, columns, null, null, null, null,
										null, null);
								String[] headers = new String[] { dbLaiXuat.COL_NAME, dbLaiXuat.COL_DATE };
								mAdapter = new SimpleCursorAdapter(LaiXuat.this, R.layout.t_custom_listlaixuat,
										mCursorlaixuat, headers,
										new int[] { R.id.tv_custom_tenlaixuat, R.id.tv_custom_ngaylaixuat });
								listlaixuat.setAdapter(mAdapter);
								dialogthu.cancel();
							}
						});
				...
				b.setNeutralButton("Xem", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						edt_sothang = (EditText) dialogthu.findViewById(R.id.EditText_xemsoluongthang); // edited
						...
						img_btn_kq.setOnClickListener(new OnClickListener() {
							...
							@Override
							public void onClick(View v) {
								if (edt_sothang.getText().toString().isEmpty()
										|| checkZero(edt_sothang.getText().toString())) {
									Toast toast = Toast.makeText(LaiXuat.this, "Bạn Chưa Nhập Số Tháng",
											Toast.LENGTH_SHORT);
									toast.show();
								} else {
									double d = kqLaiXuat(arrngay.get(position).getTien(),
											arrngay.get(position).getLaixuat(), edt_sothang.getText().toString());
				...
				b.setNegativeButton("Xóa", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						String rowId = mCursorlaixuat.getString(0); // Column 0
																	// of the
																	// cursor is
																	// the id
	...
	protected void onResume() {
		...
		String[] headers = new String[] { dbLaiXuat.COL_NAME, dbLaiXuat.COL_DATE };
		mAdapter = new SimpleCursorAdapter(LaiXuat.this, R.layout.t_custom_listlaixuat, mCursorlaixuat, headers,
				new int[] { R.id.tv_custom_tenlaixuat, R.id.tv_custom_ngaylaixuat });
		listlaixuat.setAdapter(mAdapter);
	}
	...
	public class DatePickerFragment extends DialogFragment implements DatePickerDialog.OnDateSetListener {
		...
		public void onDateSet(DatePicker view, int year, int month, int day) {
			...
			if (day < 10) {
				if (month < 10) {
					datetimeloc = "0" + String.valueOf(day) + "/" + "0" + String.valueOf(month) + "/"
							+ String.valueOf(year);
				} else {
	...
	public class DatePickerFragment1 extends DialogFragment implements DatePickerDialog.OnDateSetListener {
		...
		public void onDateSet(DatePicker view, int year, int month, int day) {
			...
			if (day < 10) {
				if (month < 10) {
					datetimesualoc = "0" + String.valueOf(day) + "/" + "0" + String.valueOf(month) + "/"
							+ String.valueOf(year);
				} else {
					datetimesualoc = "0" + String.valueOf(day) + "/" + String.valueOf(month) + "/"
							+ String.valueOf(year);
				}
			} else {
				if (month < 10) {
					datetimesualoc = String.valueOf(day) + "/" + "0" + String.valueOf(month) + "/"
							+ String.valueOf(year);
				} else {
