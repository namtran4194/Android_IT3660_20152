...
public class BaoCaoThuChi extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		img_btn_kqbaocaothang.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				dsThang();
				adapterbaocaothang = new BaoCaoThang(BaoCaoThuChi.this, R.layout.t_custom_danhsachthang, arrbaocaothang,
						imageId, arrtitlethang);
		...
		danhsachbaocaothang.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				objectthuchi = new TienThuChi();
				double a = objectthuchi.kqthang("Ăn Uống", thangnam.getText().toString() + arrcalendar.get(position),
						arrchi);
				double b = objectthuchi.kqthang("Quần Áo", thangnam.getText().toString() + arrcalendar.get(position),
						arrchi);
				double c = objectthuchi.kqthang("Cho vay", thangnam.getText().toString() + arrcalendar.get(position),
						arrchi);
				double d = objectthuchi.kqthang("Sinh Hoạt", thangnam.getText().toString() + arrcalendar.get(position),
						arrchi);
				double e = objectthuchi.kqthang("Đi Lại", thangnam.getText().toString() + arrcalendar.get(position),
						arrchi);
				double f = objectthuchi.kqthang("Trả Nợ", thangnam.getText().toString() + arrcalendar.get(position),
						arrchi);
				double g = objectthuchi.kqthang("Khác", thangnam.getText().toString() + arrcalendar.get(position),
						arrchi);
	...
	public void dsThang() {
		if (arrbaocaothang.size() != 0) {
			for (int i = 0; i < arrbaocaothang.size(); i++) {
				if (Double.parseDouble(arrbaocaothang.get(i).getTienchi()) == 0
						&& Double.parseDouble(arrbaocaothang.get(i).getTienthu()) == 0) {
	...
	public void baoCaoThangHienTai() {
		...
		// thhu-)"+doingaychi.doiThang1(arrthu.get(0).getDate()));
		for (int i = 0; i < arrthu.size(); i++) {
	...
	public void baoCaoNamHienTai() {
		doingaychi = new DoiNgay();
		// System.out.println("sum---
		// thhu-)"+doingaychi.doiNam1(arrthu.get(0).getDate()));
		for (int i = 0; i < arrthu.size(); i++) {
	...
	public void openChart(double anuong, double quanao, double chovay, double sinhhoat, double dilai, double trano,
			double khac) {
		...
		defaultRenderer.setChartTitle(" Biểu Đồ Chi ");
